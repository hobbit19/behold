...
class ServerConnection(HTTPClient):
    ...
    def handleResponse(self, data):
        ...
        # this attribute is not available in the openwrt twisted-web.. gotta workaround.
        #content_type = self.client.responseHeaders.getRawHeaders('content-type')

        # just pull it from the old-style dict, if it hits an exception just set it to something..
        try: content_type = self.client.headers.get("content-type")
        except: content_type = "application/javascript"

        print "Found content type: " + str(content_type)
